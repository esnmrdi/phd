...
def power_condition(row, alpha0, alpha1, alpha2):
    if row["POWER"] >= 0:
        val = (
            variables["ALPHA0"]
...
def calculate_fuel_consumption_rate(df, variables):
    df["VTCPFM FCR (L/H)"] = df.apply(
        power_condition,
...
for vehicle in EXPERIMENTS:
    ...
    df = calculate_fuel_consumption_rate(df, variables)
