# %%
# Add experiment ID to joint OBD/PEMS data (to detrend each experiment separately)
...
# %%
# Assign ordinal segment IDs to records
def assign_segment_id(df):
    id = 0
    df["SEGMENT_ID"] = ""
    for i in range(len(df) - 1):
        df["SEGMENT_ID"][i] = id
        # dt_current = dt.datetime.strptime(df["DATETIME"][i], "%Y-%m-%d %H:%M:%S")
        # dt_next = dt.datetime.strptime(df["DATETIME"][i + 1], "%Y-%m-%d %H:%M:%S")
        dt_current = df["DATETIME"][i]
        dt_next = df["DATETIME"][i + 1]
        delta = dt_next - dt_current
        if delta.seconds > 30:
            id += 1
    df["SEGMENT_ID"][len(df) - 1] = id
    return df
