#%% [markdown]
# ## Convert Time-based Fuel and Emission Rates to Distance-based Rates per Link
...
def save_to_Excel(links, datetime_tag, vehicle, settings):
    ...
    with pd.ExcelWriter(output_path_links, engine="openpyxl", mode="w") as writer:
        links.to_excel(writer, header=True, index=None, sheet_name="Distance-based Rates per Link")
...
def generate_links(df):
    ...
    for id in link_ids:
        ...
        for gas in ("PM", "CO2", "NO2", "NO"):
            new_row[gas + "_KGMI"] = df_slice[gas + "_KGS"].sum() / new_row["linkLength"]
